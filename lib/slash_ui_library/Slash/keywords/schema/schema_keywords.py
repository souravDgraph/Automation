...
class SchemaKeywords():
    ...
    @staticmethod
    def click_add_rules_button(browser_alias):
        """
        click add rules button
        | browser_alias |  alias of the browser |

        Example:
        | Click Add Rules Button | Browser_1 |
        
        Return:
            None
        """
        browser = BrowserKeywords.switch_browser(browser_alias)
        browser.click_element(SchemaLocators.auth_rules, timeout=SchemaKeywords.timeout)
        browser.wait_until_page_contains_element(SchemaLocators.rules_textbox.replace("%s", "Query"), timeout=SchemaKeywords.timeout)
        browser.wait_until_page_contains_element(SchemaLocators.rules_textbox.replace("%s", "Add"), timeout=SchemaKeywords.timeout)
        browser.wait_until_page_contains_element(SchemaLocators.rules_textbox.replace("%s", "Update"), timeout=SchemaKeywords.timeout)
        browser.wait_until_page_contains_element(SchemaLocators.rules_textbox.replace("%s", "Delete"), timeout=SchemaKeywords.timeout)
    ...
    def click_update_button(browser_alias):
        ...
        Example:
        | Click Update Button | Browser_1 |
        
        Return:
            None
        """
        browser = BrowserKeywords.switch_browser(browser_alias)
        browser.click_element(SchemaLocators.update_button, timeout=SchemaKeywords.timeout)

    @staticmethod
    def add_rules(browser_alias, rule_type, rules):
        """
        add rules for the type
        | browser_alias |  alias of the browser |
        | rule_type | type of the rule |
        | rules | rules for the type |

        Example:
        | Add Rules | Browser_1 | Query | query($USER: String!) { queryTask { user(filter: { username: { eq: $USER } }) {  __typename } } } |
        
        Return:
            None
        """
        browser = BrowserKeywords.switch_browser(browser_alias)
        browser.input_text(SchemaLocators.rules_textbox.replace("%s", rule_type), rules)

    @staticmethod
    def select_type(browser_alias, type_name):
        """
        select the type
        | browser_alias |  alias of the browser |
        | type_name | name of the type to be selected |

        Example:
        | Select Type | Browser_1 | User |
        
        Return:
            None
        """
        browser = BrowserKeywords.switch_browser(browser_alias)
        browser.click_element(SchemaLocators.select_type.replace("%s", type_name), timeout=SchemaKeywords.timeout)
        browser.wait_until_page_contains_element(SchemaLocators.auth_rules, timeout=SchemaKeywords.timeout)
