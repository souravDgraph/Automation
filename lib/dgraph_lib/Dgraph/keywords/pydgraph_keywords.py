...
from lib.dgraph_lib.Dgraph.components.pydgraph_configuration import PydgraphOperations
...
class PydgraphKeywords:
    ...
    def pyd_create_connection(self, address):
        """
    ...
    def pyd_create_mtls_connection(self, address, loc=None, username=None, password=None):
        """
        ...
        if loc and username and password:
            res_client = self.py_client.create_custom_client_connect_with_login(address, loc, username, password)
        else:
            res_client = self.py_client.create_custom_client_connect_with_login(address)
        return res_client
